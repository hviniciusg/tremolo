...
static int _fetch_headers(OggVorbis_File *vf,
	...
	int i,ret;
	fprintf(stderr,"fetching headers _fetch_headers \n");
	...
	while(i<3){
		...
		while(i<3){
			fprintf(stderr,"inside the second loop of love \n");
			int result=ogg_stream_packetout(vf->os,&op);
			if(result==0){
				fprintf(stderr,"ogg_strem_packetout == 0 \n");
				break;
			}
			if(result==-1){
				fprintf(stderr,"ogg_strem_packetout == -1  \n");
				ret=OV_EBADHEADER;
				goto bail_header;
			}
...
static int _set_link_number(OggVorbis_File *vf,int link){
	fprintf(stderr,"Inside _set_link_number \n");
...
int ov_pcm_seek(OggVorbis_File *vf,ogg_int64_t pos){
	fprintf(stderr,"inside ov_pcm_seek \n");
	ogg_packet op={0,0,0,0,0,0};
	ogg_page og={0,0,0,0};
	...
	if(_make_decode_ready(vf)){
		fprintf(stderr,"Not make decode ready, return OV_EBADLINK");
		return OV_EBADLINK;
	}
	 ...
	 length positioning. */
			...
			if(vf->ready_state<STREAMSET){
				...
				for(link=0;link<vf->links;link++)
					if(vf->serialnos[link]==vf->current_serialno)break;
				if(link==vf->links){
					fprintf(stderr,"Links==vf->links, return OV_EBADLINK");
					ogg_page_release(&og);
					ogg_packet_release(&op);
					return OV_EBADLINK;
				}
				...
				vf->current_link=link;
				ret=_fetch_headers(vf,&vf->vi,&vf->vc,&vf->current_serialno,&og);
				fprintf(stderr,"we fucking called _fetch_headers with ret: %i",ret);
				if(ret) return ret;
				if(_make_decode_ready(vf))return OV_EBADLINK;
				lastblock=0;
			}
	...
	fprintf(stderr,"we got out of the while loop :(");
	vf->bittrack=0;
	vf->samptrack=0;
...
int ov_time_seek(OggVorbis_File *vf,ogg_int64_t milliseconds){
	...
	fprintf(stderr,"inside ov_time_seek \n");
	int link=-1;
