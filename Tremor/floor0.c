...
vorbis_info_floor *floor0_info_unpack (vorbis_info *vi,oggpack_buffer *opb){
	...
	info->numbooks=oggpack_read(opb,4)+1; //NOTE: posible plase to change size
...
ogg_int32_t *floor0_inverse1(vorbis_dsp_state *vd,vorbis_info_floor *i,
	...
	vorbis_info_floor0 *info=(vorbis_info_floor0 *)i;
	int j,k;
	fprintf(stderr,"We got to the lucky charm one: floor0_inverse1 \n");
	...
	if(ampraw>0){ /* also handles the -1 out of data case */
		...
		if(booknum!=-1 && booknum<info->numbooks){ /* be paranoid */
			...
			for(j=0;j<info->order;){
				//fprintf(stderr,"Probably here is the OORW????? \n");
				//modify dim to cause obw
