...
long _book_maptype1_quantvals(codebook *b){
	...
	fprintf(stderr,"Inside _book_maptype1_quantvals \n");
	...
	while(1){
		...
		int i;
		for(i=0;i<b->dim;i++){
			if (acc > b->entries / vals) fprintf(stderr,"Este no cumple con el parche!!! acc > b->entries / vals \n ");
			acc*=vals;
			if (acc1 > LONG_MAX / (vals + 1)) fprintf(stderr,"Este no cumple con el parche!!! acc1 > LONG_MAX / (vals + 1) \n");
			acc1*=vals+1;
		}
		if (i >= b->dim && acc <= b->entries && acc1 > b->entries) fprintf(stderr,"Este no cumple con el parche!!! i >= b->dim && acc <= b->entries && acc1 > b->entries)\n");
		if(acc<=b->entries && acc1>b->entries){
			return(vals);
		}else{
			if (i < b->dim || acc > b->entries) fprintf(stderr,"Este no cumple con el parche!!! i < b->dim || acc > b->entries\n");
			if(acc>b->entries){
				vals--;
			}else{
				vals++;
...
int vorbis_book_unpack(oggpack_buffer *opb,codebook *s){
	...
	memset(s,0,sizeof(*s));
	fprintf(stderr,"inside vorbis_book_unpack unpaking codebook \n");
	...
	case 0:
		fprintf(stderr,"codeword Ordering: Unordered \n");
		/* unordered */
	...
	case 1:
		/* ordered */
		fprintf(stderr,"codeword Ordering: Ordered \n");
		{
	...
	if((maptype=oggpack_read(opb,4))>0){
		fprintf(stderr,"---- We have maptype to unpack maptype= %i --- \n",maptype);
	...
	case 1:
		fprintf(stderr,"----- maptype 1 decoding\n ---- ");
		/* mapping type 1; implicit values by lattice  position */
		quantvals=_book_maptype1_quantvals(s);

		fprintf(stderr,"quantvals = %i, s->dim = %ld s->entries = %ld \n",quantvals,s->dim,s->entries);
	...
	_errout:
	_eofout:
	fprintf(stderr,"_errout or _eofout shit! \n");
	vorbis_book_clear(s);
	return -1;
}
...
long vorbis_book_decodev_set(codebook *book,ogg_int32_t *a,
	...
	if(book->used_entries>0){
		...
		for(i=0;i<n;){
			if(decode_map(book,b,v,point))return -1;
			for (j=0;j<book->dim;j++){
				fprintf(stderr,"and here Probably is the OORW too:vorbis_book_decodev_set????? \n");
				a[i++]= v[j];}
		}
	}else{
		int i,j;
